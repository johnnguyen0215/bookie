    ...
    User.findOne({ google: profile.id }, function(err, existingUser) {
      if (existingUser) {
        ...
        User.findById(req.user.id, function(err, user) {
          ...
          user.profile.name = user.name || profile.displayName;
          user.profile.picture = user.picture || profile._json.picture;
          user.profile.campus = req.body.profile.campus;
    ...
    User.findOne({ google: profile.id }, function(err, existingUser) {
      ...
      User.findOne({ email: profile._json.emails[0].value }, function(err, existingEmailUser) {
        if (existingEmailUser) {
          ...
          user.profile.campus = req.query.state;
          user.save(function(err) {
            done(err, user);
          });
        }
